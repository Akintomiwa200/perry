...
function ProductCard({ product, onSelect }: { product: any; onSelect: (id: number) => void }) {
  ...
  return (
    ...
    >
      {/* Image Preview */}
      <div
        className="w-full h-40 rounded-lg mb-3 flex items-center justify-center overflow-hidden"
      ...
      <div className="space-y-1.5">
        ...
        <div className="flex items-center justify-between pt-1">
          <span className="text-sm font-bold" style={{ color: 'var(--deep)' }}>
...
function ProductRow({ product, idx, onSelect }: { product: any; idx: number; onSelect: (id: number) => void }) {
  ...
  return (
    <tr
      ...
      style={{
        background: 'var(--color-surface-raised)',
        borderBottom: idx < idx ? '1px solid var(--color-border)' : undefined,
      }}
    >
  ...
  return (
    <div className="flex flex-col gap-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex rounded-lg overflow-hidden" style={{ border: '1px solid var(--color-border)' }}>
            <button
              ...
              style={{ background: viewMode === 'table' ? 'var(--deep)' : 'var(--color-surface-raised)', color: viewMode === 'table' ? 'var(--cream)' : 'var(--color-text)' }}
            >
              Table
            </button>
            <button
              ...
              className="px-3 py-1.5 text-xs font-medium transition"
              style={{ background: viewMode === 'grid' ? 'var(--deep)' : 'var(--color-surface-raised)', color: viewMode === 'grid' ? 'var(--cream)' : 'var(--color-text)' }}
            >
              Grid
            </button>
          </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <button
          className="h-10 px-4 text-sm rounded-lg font-medium transition hover:opacity-80"
        ...
        <button
          className="h-10 px-4 text-sm rounded-lg font-medium transition hover:opacity-80"
          style={{ background: 'var(--color-surface-raised)', border: '1px solid var(--color-border)' }}
          onClick={() => { setSearch(''); setCategory(''); setStatus('all'); setSearchInput(''); }}
        >
          Clear
        </button>
      </div>
       ...
       >
         <table className="w-full text-left border-collapse">
           ...
           <tbody>
             ...
             {!isLoading && !error && products.map((product, idx) => (
