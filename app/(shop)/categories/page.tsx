          ...
          {CATEGORIES.map((cat) => (
            <Link
              ...
              style={{
                ...
                padding: '2.5rem 1.5rem', textAlign: 'center', background: 'var(--cream)',
                border: '1px solid var(--blush)', borderRadius: 4, textDecoration: 'none',
                transition: 'all .3s',
              }}
            >
              <span style={{ fontSize: '3rem' }}>{cat.icon}</span>
              <span style={{ fontSize: '1rem', fontWeight: 500, color: 'var(--deep)' }}>{cat.name}</span>
            </Link>
          ))}
        </div>
      </main>
